...
def classify_hand(hand):
    counts = Counter(hand)
    if sorted(list(counts.values())) == [5]:
        order = 1
    elif sorted(list(counts.values())) == [1, 4]:
        order = 2
    elif sorted(list(counts.values())) == [2, 3]:
        order = 3
    elif sorted(list(counts.values())) == [1, 1, 3]:
        order = 4
    elif sorted(list(counts.values())) == [1, 2, 2]:
        order = 5
    elif sorted(list(counts.values())) == [1, 1, 1, 2]:
        order = 6
    else:
        order = 7
    return order
